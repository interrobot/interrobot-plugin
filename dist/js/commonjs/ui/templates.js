...
class Templates {
    ...
    static standardHeading(project, title) {
        return `<div class="main__heading">
            ...
            <div class="main__heading__title">
                <h1><span>${html_js_1.HtmlUtils.htmlEncode(title)}</span></h1>
    ...
    static cellRendererLinkedId(cellValue, rowData, i) {
        ...
        const result = {
            "classes": [],
            "content": `<a tabindex="0" href="${html_js_1.HtmlUtils.htmlEncode(interrobotPageDetail)}"
                data-id="${html_js_1.HtmlUtils.htmlEncode(rowData["ID"])}" 
                class="ulink">${html_js_1.HtmlUtils.htmlEncode(cellValue)}</a>`
        };
        return result;
    }
    /**
